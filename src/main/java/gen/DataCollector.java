...
public class DataCollector {
	...
	public ResultSet getArtist(Connection conn) {
		ResultSet res = null;
		try {
			String query = "select id,artist,artist_img_url,description from tbl_artist where status=2";
	...
	public ResultSet getAllVideos(Connection conn, String id) {
		ResultSet res = null;
		try {
			String query = "select vurl,videoid,imgurl,video_name from tbl_videos where artist='"+id+"' and status=2";
			System.out.println(query);
			PreparedStatement ps = conn.prepareStatement(query);
			res = ps.executeQuery();
		} catch (Exception e) {
	...
	public ResultSet getArtistName(Connection conn, String id) {
		ResultSet res = null;
		try {
			String query = "select name from tbl_videos where artist='"+id+"' and status=2";
			System.out.println(query);
			PreparedStatement ps = conn.prepareStatement(query);
			res = ps.executeQuery();
		} catch (Exception e) {
	...
	public ResultSet getVideoDetail(Connection conn, String vid) {
		ResultSet res = null;
		try {
			String query = "select vurl,name,video_name from tbl_videos where videoid='"+vid+"' and status=2";
			System.out.println(query);
			PreparedStatement ps = conn.prepareStatement(query);
			res = ps.executeQuery();
		} catch (Exception e) {
	...
	public ResultSet getRelatedVideos(Connection conn, String id) {
		ResultSet res = null;
		try {
			String query = "select vurl,videoid,imgurl,video_name from tbl_videos where artist='"+id+"' and status=2 order by RAND() limit 4";
			System.out.println(query);
			PreparedStatement ps = conn.prepareStatement(query);
			res = ps.executeQuery();
		} catch (Exception e) {
	...
	public String getStatus(String ani) {
		...
		try {
			...
			final String chkqry = "select * from tbl_subscription where ani='" + ani + "' and service_type='Diski 99' ";
			System.out.println(chkqry);
			final ResultSet rs = stmt.executeQuery(chkqry);
			if (rs.next()) {
				State = this.getUserState(ani, "Diski 99");
			} else {
				State = "2";
			}
		} catch (Exception e) {
